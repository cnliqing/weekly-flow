...
周报通是一个多项目周报系统，支持在首页选择或创建项目。进入项目后，可维护当前项目成员与角色、创建周报周期、收集成员免登录提交、查看计划承接提示、执行 AI 汇总/润色、保存定稿并查看历史记录。项目之间的数据按项目上下文隔离。
...
首次安装依赖后运行 `npm run prisma:generate` 生成 Prisma Client；数据库 schema 就绪后可运行 `npm run db:seed` 写入示例项目、成员和当前周报周期。

## 工作台

访问 `/` 进入项目门户，可选择已有项目或创建新项目。创建项目时可同时设定团队成员和角色，角色包含项目经理与开发。

进入 `/projects/[projectId]` 后维护当前项目的成员、周报周期、汇总定稿、历史记录和项目数据。旧 `/admin` 入口会回到项目门户。系统不包含后台登录、注册或用户表；成员从 `/w` 按项目选择开放周报周期和姓名后填写。
